...
namespace ApiProject.WebUI.Controllers
{
    public class WhyChooseYummyController : Controller
    {
        ...
        public async Task<IActionResult> WhyChooseYummyList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7072/api/Services");
        ...
        public async Task<IActionResult> CreateWhyChooseYummy(CreateWhyChooseYummyDto createWhyChooseYummyDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7072/api/Services", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7072/api/Services?id=" + id);
            return RedirectToAction("WhyChooseYummyList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7072/api/Services/GetService?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateWhyChooseYummy(UpdateWhyChooseYummyDto updateWhyChooseYummyDto)
        {
            ...
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7072/api/Services/", content);
            return RedirectToAction("WhyChooseYummyList");
        }
    }
}
