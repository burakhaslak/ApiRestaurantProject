...
namespace ApiProject.WebUI.Controllers
{
    public class GalleryController : Controller
    {
        ...
        public async Task<IActionResult> ImageList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7072/api/Images");
        ...
        public async Task<IActionResult> CreateImage(CreateImageDto createImageDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7072/api/Images", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7072/api/Images?id=" + id);
            return RedirectToAction("ImageList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7072/api/Images/GetImage?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateImage(UpdateImageDto updateImageDto)
        {
            ...
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7072/api/Images/", content);
            return RedirectToAction("ImageList");
        }
    }
}
