...
namespace ApiProject.WebUI.Controllers
{
    public class YummyEventController : Controller
    {
        ...
        public async Task<IActionResult> YummyEventList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7072/api/YummyEvents");
        ...
        public async Task<IActionResult> CreateYummyEvent(CreateYummyEventDto createYummyEventDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7072/api/YummyEvents", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7072/api/YummyEvents?id=" + id);
            return RedirectToAction("YummyEventList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7072/api/YummyEvents/GetYummyEvent?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateYummyEvent(UpdateYummyEventDto updateYummyEventDto)
        {
            ...
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7072/api/YummyEvents/", content);
            return RedirectToAction("YummyEventList");
        }
    }
}
