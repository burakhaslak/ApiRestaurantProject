...
namespace ApiProject.WebUI.Controllers
{
    public class ChefController : Controller
    {
        ...
        public async Task<IActionResult> ChefList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7072/api/Chefs");
        ...
        public async Task<IActionResult> CreateChef(CreateChefDto createChefDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7072/api/Chefs", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7072/api/Chefs?id=" + id);
            return RedirectToAction("ChefList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7072/api/Chefs/GetChef?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateChef(UpdateChefDto updateChefDto)
        {
            ...
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7072/api/Chefs/", content);
            return RedirectToAction("ChefList");
        }
    }
}
